...
pub fn mdka(args: &[String]) -> Result<()> {
    let base_args = vec!["run", "--bin", "mdka-cli", "--"];

    let all_args = [base_args, args.iter().map(AsRef::as_ref).collect()].concat();

    duct::cmd("cargo", &all_args).run()?;
